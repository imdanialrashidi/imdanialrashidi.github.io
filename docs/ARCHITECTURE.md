...
- **Test entrypoints:** `node --test tests/projects.test.mjs` (product helpers, frontmatter safety, draft filtering, shared prop usage — no `astro:content` import). Harness suites: `node --test tests/*.test.mjs` (96 tests at baseline); gate via `bash scripts/verify.sh` which runs `bash scripts/pi-doctor.sh --ci` then `npm run ci` (`check && build`) or `format:check`/`lint` fallbacks.
