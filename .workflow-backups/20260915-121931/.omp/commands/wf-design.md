...
1. Read `docs/PRODUCT.md`, `docs/DESIGN.md`, `docs/QUALITY.md`, and the relevant existing UI.
   Run `node scripts/validate-project-context.mjs --static`; do not invent an audience, brand, locale, or budget to fill an empty contract.
2. Read `docs/VISUAL_REVIEW.md`; use OMP's bundled `designer` only when a separate specialist context adds value, and load `browser-qa` when an existing rendered interface can be inspected.
...
7. Run the anti-template check from `docs/VISUAL_REVIEW.md` and revise any interchangeable choice.
...
Use `accessibility-audit`, `web-performance`, or `rtl-i18n` only when the accepted direction makes that domain material; their evidence contracts supplement OMP's native designer/browser, they do not replace them.
