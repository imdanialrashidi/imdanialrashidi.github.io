...
const allowedProductPrefixes = Object.keys(pkg.devDependencies ?? {}).map((n) => `npm:${n}@`)
for (const source of entries.keys()) {
  if (configured.includes(source)) continue
  if (allowedProductPrefixes.some((p) => source.startsWith(p))) continue
...
if (online) {
  for (const [source, entry] of entries) {
    ...
    const published = Array.isArray(raw) ? raw[0] : raw
    const distIntegrity = published?.['dist.integrity'] ?? published?.dist?.integrity
    if (distIntegrity !== entry.integrity) {
      throw new Error(
        `registry integrity mismatch for ${source}: reviewed=${entry.integrity} published=${distIntegrity}`,
      )
    }
    if (published.license !== entry.license)
      throw new Error(
        `registry license mismatch for ${source}: reviewed=${entry.license} published=${published.license}`,
      )
